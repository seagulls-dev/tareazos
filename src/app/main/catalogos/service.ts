...
export class Service implements Resolve<any>
{
    ...
    get(): Promise<any>
    {
        ...
        return new Promise((resolve, reject) => {
            this.firestore.collection('forms').doc(this.filterBy).valueChanges().subscribe((snapshot) => {
                var dataModel = snapshot;

                this.rows = [];

                this.firestore.collection('forms').doc(this.filterBy).collection('campos').valueChanges().subscribe((snapshotCampos) => {
                    this.rows.campos = [];
                    this.campos = [];
                    ...
                    this.rows.campos = this.campos.sort(function(a, b){return a.order - b.order});

                    if ( this.searchText && this.searchText !== '' )
                    {
                        this.rows.campos = FuseUtils.filterArrayByString(this.rows.campos, this.searchText);
                    }
    ...
    {
        return new Promise((resolve, reject) => {
            this.firestore.collection('forms').doc(formId).collection('campos').doc(model.id).set(model).then(response => {
                resolve(response);
            });
        });
    }
    ...
    {
        return new Promise((resolve, reject) => {
            this.firestore.collection('forms').doc(formId).collection('campos').add(model).then(response => {
    ...
    {
        this.firestore.collection('forms').doc(formId).collection('campos').doc(model.id).delete().then(() => {
        }, (error) => {
          console.error(error);
        });
    }
    ...
    deleteSelected(formId): void
    {
        ...
        {
            this.firestore.collection('forms').doc(formId).collection('campos').doc(modelId).delete().then(() => {
            }, (error) => {
              console.error(error);
            });
        }
